...
LDCLFLAGS += --config pointer_chasing.ini 
EXECUTABLE = host
CMD_ARGS = $(BUILD_DIR)/pointer_chasing.xclbin
EMCONFIG_DIR = $(TEMP_DIR)
EMU_DIR = $(SDCARD)/data/emulation

BINARY_CONTAINERS += $(BUILD_DIR)/pointer_chasing.xclbin
BINARY_CONTAINER_pointer_chasing_OBJS += $(TEMP_DIR)/pointer_chasing.xo
...
# Building kernel
$(TEMP_DIR)/pointer_chasing.xo: src/pointer_chasing.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k pointer_chasing -I'$(<D)' -o'$@' '$<'
$(BUILD_DIR)/pointer_chasing.xclbin: $(BINARY_CONTAINER_pointer_chasing_OBJS)
	mkdir -p $(BUILD_DIR)
...
ifeq ($(HOST_ARCH), x86)
	...
	XCL_EMULATION_MODE=$(TARGET) ./$(EXECUTABLE) $(BUILD_DIR)/pointer_chasing.xclbin
else
	mkdir -p $(EMU_DIR)
...
ifeq ($(HOST_ARCH), x86)
	./$(EXECUTABLE) $(BUILD_DIR)/pointer_chasing.xclbin
endif
endif
